...
import { notifyUserAboutCaloriesReminder } from "@/lib/tracking-reminder-notifications";
...
export async function processDueCaloriesReminders(now = new Date()) {
  ...
  const reminders = await prisma.caloriesReminder.findMany({
    where: { enabled: true },
    select: {
      id: true,
      traineeId: true,
  ...
  for (const reminder of reminders) {
    ...
    const loggedToday = await prisma.caloriesLog.findFirst({
    ...
    const delivered = await notifyUserAboutCaloriesReminder({
