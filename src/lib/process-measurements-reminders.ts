...
import { notifyUserAboutMeasurementsReminder } from "@/lib/tracking-reminder-notifications";
...
function hasAnyMeasurement(log: Record<string, unknown> | null) {
  if (!log) return false;
  return MEASUREMENT_FIELDS.some((field) => log[field.key] != null);
}

export async function processDueMeasurementsReminders(now = new Date()) {
  ...
  const reminders = await prisma.measurementsReminder.findMany({
    where: { enabled: true },
    select: {
      id: true,
      traineeId: true,
  ...
  for (const reminder of reminders) {
    ...
    const loggedToday = await prisma.measurementsLog.findFirst({
      where: { traineeId: reminder.traineeId, recordedDay: today },
    });

    if (hasAnyMeasurement(loggedToday)) {
      skipped += 1;
      continue;
    }

    const delivered = await notifyUserAboutMeasurementsReminder({
