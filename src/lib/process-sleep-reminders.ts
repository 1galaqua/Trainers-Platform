...
import { notifyUserAboutSleepReminder } from "@/lib/tracking-reminder-notifications";
...
export async function processDueSleepReminders(now = new Date()) {
  ...
  const reminders = await prisma.sleepReminder.findMany({
    where: { enabled: true },
    select: {
      id: true,
      traineeId: true,
  ...
  for (const reminder of reminders) {
    ...
    const loggedToday = await prisma.sleepLog.findFirst({
    ...
    const delivered = await notifyUserAboutSleepReminder({
