...
import { notifyUserAboutStepsReminder } from "@/lib/tracking-reminder-notifications";
...
export async function processDueStepsReminders(now = new Date()) {
  ...
  const reminders = await prisma.stepsReminder.findMany({
    where: { enabled: true },
    select: {
      id: true,
      traineeId: true,
  ...
  for (const reminder of reminders) {
    ...
    const loggedToday = await prisma.stepsLog.findFirst({
    ...
    const delivered = await notifyUserAboutStepsReminder({
