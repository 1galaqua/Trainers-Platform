...
import { computeSleepHours, formatSleepRange } from "@/lib/sleep-validation";
import { deleteSleepLogAction, type SleepLogItem } from "@/server/actions/sleep";

type SleepHistoryProps = {
  logs: SleepLogItem[];
};

export function SleepHistory({ logs }: SleepHistoryProps) {
  ...
  async function handleDelete(logId: string) {
    setLoadingId(logId);
    setError(null);
    const result = await deleteSleepLogAction(logId);
    setLoadingId(null);
